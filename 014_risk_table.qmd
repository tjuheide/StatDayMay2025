...
At this point `base_size` actually does not affect anything. The size of the text can be modified by the parameter `size` inside `geom_text()`. `base_size` is not used until we reinsert the labels on the y-axis.
:::
