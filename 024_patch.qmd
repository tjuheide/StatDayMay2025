...
Assuming you have created objects `Plague_curves`, `Plague_table`, `Cholera_curves`, and `Cholera_table`  (the names will be used below, but use whichever makes sense to you), we can now patch these together. First we will make risk curves with risk tables for one outcome, then for the other, then we will combine everything.
...
However, when we add annotation it is important to have the code combining the plots in parentheses, so be sure to do that.
...
The tags are perhaps a bit far from the plots (the annotation for the second panel is perhaps too close to the first panel). We can move the tags by adding a custom theme to the above plot. Note that here we need to use `&` instead of `+` when we add this custom theme to get it to work on both plots.
