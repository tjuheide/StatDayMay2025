...
In `SAS` we have macros, `R` lets us use function. Here we aim to develop a function which will let us make a risk plot for a specified value of `analysis` in our `risk` data set.
...
Then apply `filter()` where you restrict to `analysis` equalling `thisanalysis`
:::
...
Finally, remove the legend title by setting `legend.title` to `element_blank()` (you also need to do this inside your custom theme).
...
Apply the function on the two different analyses (`"Cholera"` and `"Plague"`). Notice that the y-axes have different scales. Use `coord_cartesian()` inside the function to make the y-axis go to 15% (0.15).
